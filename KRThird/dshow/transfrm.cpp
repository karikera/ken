...
// then we return nullptr, the assumption being that the caller will realise the
...
// creating the pin if need be.  Otherwise return nullptr with an error.
...
CTransformFilter::Stop()
{
    ...
    ASSERT(m_pInput == nullptr || m_pOutput != nullptr);
    if (m_pInput == nullptr || m_pInput->IsConnected() == FALSE ||
...
CTransformFilter::Pause()
{
    ...
    else if (m_pInput == nullptr || m_pInput->IsConnected() == FALSE) {
...
{
    ASSERT(m_pTransformFilter->m_pOutput != nullptr);
...
CTransformInputPin::CheckMediaType(const CMediaType* pmt)
{
    ...
    if ((m_pTransformFilter->m_pOutput != nullptr) &&
...
CTransformOutputPin::NonDelegatingQueryInterface(REFIID riid, void **ppv)
{
    ...
    *ppv = nullptr;
    ...
    if (riid == IID_IMediaPosition || riid == IID_IMediaSeeking) {
        ...
        ASSERT(m_pTransformFilter->m_pInput != nullptr);
