...
CDisp::CDisp(IPin *pPin)
{
    ...
    if (pPin) {
       ...
       (void)StringCchCopy(str, NUMELMS(str), TEXT("nullptr IPin"));
    }
...
void WINAPI DisplayType(LPTSTR label, const AM_MEDIA_TYPE *pmtIn)
{
	  ...
	  && (pmtIn->cbFormat >= sizeof(PCMWAVEFORMAT)))
	{
	    ...
	    WAVEFORMATEX *pwfx = (WAVEFORMATEX *) pmtIn->pbFormat;
            DbgLog((LOG_TRACE,2,TEXT("wFormatTag %L"), pwfx->wFormatTag));
            DbgLog((LOG_TRACE,2,TEXT("nChannels %L"), pwfx->nChannels));
            ...
            DbgLog((LOG_TRACE,2,TEXT("nBlockAlign %L"), pwfx->nBlockAlign));
            DbgLog((LOG_TRACE,2,TEXT("wBitsPerSample %L"), pwfx->wBitsPerSample));

            /* PCM uses a WAVEFORMAT and does not have the extra size field */

            if (pmtIn->cbFormat >= sizeof(WAVEFORMATEX)) {
                DbgLog((LOG_TRACE,2,TEXT("cbSize %L"), pwfx->cbSize));
            }
	} else {
	}
...
void WINAPI DumpGraph(IFilterGraph *pGraph, DWORD dwLevel)
{
	...
	if (FAILED(pFilter->QueryFilterInfo(&info))) {
		...
		while (pins->Next(1, &pPin, &n) == S_OK) {
		    ...
		    if (FAILED(pPin->QueryPinInfo(&pin_info))) {
			...
			if (pPinConnected) {
			    DbgLog((LOG_TRACE,dwLevel,TEXT("          Pin [%x]  '%ls' [%sput]")
				    ...
				    pPin, pin_info.achName,
				    pin_info.dir == PINDIR_INPUT ? TEXT("In") : TEXT("Out"),
				    pPinConnected));
