...
namespace kr
{
	...
	#define _RETURN_(type)		ATTR_CHECK_RETURN ATTR_NOALIAS static type
	#define _NULLABLE_(type)	ATTR_NULLABLE _RETURN_(type)
	#define _NOTNULL_(type)		ATTR_NONULL _RETURN_(type)
	namespace _pri_
	{
		template <typename T>
		struct _tconst
	...
	namespace _pri_
	{
		...
		template <> struct ARRCOPY<true, true>
		{
			...
			template <typename T> _VOID_ ctor(void * dest, size_t size) noexcept;
			template <typename T> _VOID_ dtor(void * dest, size_t size) noexcept;
		...
		template <> struct ARRCOPY<true, false>
		{
			...
			template <typename T> _VOID_ ctor(T * dest, size_t size) noexcept;
			template <typename T> _VOID_ dtor(T * dest, size_t size) noexcept;
