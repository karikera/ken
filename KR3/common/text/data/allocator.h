...
namespace kr
{
	namespace ary
	{
		...
		public:
			...
			{
				StackAllocator * allocator = StackAllocator::getInstance();
				byte * buffer = (byte*)p - offset;
				if (allocator->isLastBlock(buffer))
				{
					return allocator->expand(buffer, sz * sizeof(InternalComponent) + offset);
				}
				else
				{
			...
			static InternalComponent* _obj_move(InternalComponent * from, size_t ncap, LAMBDA && ctor_move_d) throws(NotEnoughSpaceException)
			{
				StackAllocator * allocator = StackAllocator::getInstance();
				...
				InternalComponent* narr = (InternalComponent*)((byte*)allocator->allocateWithNewNode(ncap + offset, align, offset) + offset);
				ctor_move_d(narr);
		...
		public:
			...
			static void* _mem_base(InternalComponent* p) noexcept
			{
				return (byte*)p - offset;
			}

			template <typename LAMBDA>
			static InternalComponent* _obj_move(InternalComponent * from, size_t ncap, LAMBDA && ctor_move_d) noexcept
			{
				ondebug(_pri_::AllocationMove _temp((byte*)from-offset));
